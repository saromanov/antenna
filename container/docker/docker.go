...
func (d *Docker) toStats(stats *docker.Stats) *structs.ContainerStat {
	return &structs.ContainerStat{
		CPU: structs.CPUStat{
			...
			OnlineCPUs:      int64(stats.CPUStats.OnlineCPUs),
			SystemCPUUsage:  int64(stats.CPUStats.SystemCPUUsage),
			UsageInUsermode: int64(stats.CPUStats.CPUUsage.UsageInUsermode),
		},
		NumProcs:       int32(stats.NumProcs),
		...
		ReadSizeBytes:  int64(stats.StorageStats.ReadSizeBytes),
		WriteSizeBytes: int64(stats.StorageStats.WriteSizeBytes),
		TxPackets:      int64(stats.Network.TxPackets),
		TxDropped:      int64(stats.Network.TxDropped),
		TxBytes:        int64(stats.Network.TxBytes),
		TxErrors:       int64(stats.Network.TxErrors),
	}
}
